...
head(loc.mor)
```
Because each local Moran's I is calculated solely based on the few neighbours of a given site, the exact test is much more appropriate in this case.
...
The interpretation is similar as with the global I: high positive values indicate those sites with species richness highly similar to that of its neighbours; high negative values (rarely present) indicate negatively correlated values; and values near zero indicate sites that are independent from their neighbourhood. The values, however, are no longer bounded in -1 and 1, and the interpretation of the absolute values is unclear. For this reason, it is also useful to depict the p values: the values near 0 indicate sites that are correlated with their neighbourhood, which typically means they have similar richness values. Such areas can be called "clusters". On the other hand, the p values near 1 indicate site that are independent of their neighbourhoods, which often leads to differing richness values and can depict "hotspots". 
